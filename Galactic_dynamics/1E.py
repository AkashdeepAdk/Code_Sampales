#This file calculates the entropy of each cell
import numpy as np
...
del all_CellName[100:]  #this file calculates for first 100 cells
cell_ent=[]
orb_ent=[]
weights=[]
...
for i in lines:
    c=c+1
    if c>1:
        weights.append(float(i.split()[6]))   #creates weights vector
...
for fnum in range(1,11):
    ...
    for i in lines2:
        c1=c1+1
        if c1>1:
            orb_ent.append(float(i.split()[1]))  #creates orbit entropy vector i.e. this array contains entropy of all the orbits
...
for cl in range(100):
    ...
    #calculation of cell entropies using the formula suggested by Wozniak and Pfenniger
...
ascii.write(data, 'cell_wise_entropy_1.txt', overwrite=True,delimiter=' ')